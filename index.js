...
function sortCarInventory(arr) {
  return arr.sort(function(par1, par2) {
    if(par1.car_model > par2.car_model) {
      return 1;
    }
    return -1;
  });
...
*/
function getOlderCars(arr2, num) {
  let olderCars = []
  for (var i = 0; i < arr2.length; i++) {
    if (arr2[i].car_year <= num) {
      olderCars.push(arr2[i]);
    }
  }
  return olderCars;
}
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(arr3) {
  let germanCars = []
  for (var i = 0; i < arr3.length ; i++) {
    if (arr3[i].car_make === "Audi" || arr3[i].car_make === "Mercedes-Benz" ||arr3[i].car_make === "Volkswagen"  ||arr3[i].car_make === "BMW" )
    germanCars.push(arr3[i]);
  }
...
function carMaker(num) {
  const obj = {
    odometer: num,
    drive:(distance) => obj.odometer += distance
}
return obj;
}
